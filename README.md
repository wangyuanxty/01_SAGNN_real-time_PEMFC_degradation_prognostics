...
神经网络四层分别对应公式中的不同部分：**指数衰减核**（`w_la` 参数，控制退化速度）、**特征编码层**（`Linear(5→1)`，5 个运行参数映射为驱动项）、**Sigmoid 隐藏层**（10 节点，非线性变换）、**线性输出层**（产生 AGO 空间的退化预测值）。训练完成后，指数衰减核参数和特征编码层权重按公式 (6) 提取为灰系数 $ a $ 和 $ b_1 \dots b_5 $。
...
**时变衰减率。** 将常数 $ a $ 扩展为 $ a(k) = \mathrm{softplus}(−w_{\mathrm{la}} + \mathrm{net}(k)) $。动机是论文 §5 (line 585) 的 "deeper neural architectures for enhanced temporal modelling"，但未作为正式开放问题。结果 MAPE 1.10%，劣于 baseline——6 个训练 k 值不足以训练有意义的时变函数，确认了该方向在数据量更大时才有实际价值。

**Neural ODE。** 用连续时间微分方程 $ dx/dt = −a·x + \Sigma b·u + \mathrm{net}(t,x) $ 替代离散灰组合，RK4 积分。天然处理不等间隔时间点（论文特征化时刻间隔 35–185h）。纯灰 ODE（6 参数）MAPE 0.49%，加网络校正（343 参数）MAPE 0.52%。理论上优雅但数值积分慢，作为备选方案保留在 `model_node.py`。
...
**深层特征编码可行因为灰模板约束。** 灰组合 $ (x0 − \text{驱动项}) · e^{−a·k} + \text{驱动项} $ 将特征编码的输出空间强约束为一维标量。363 个参数的有效自由度远小于形式上的数量。
