...
**时变衰减率。** $a$ → 时变 $a(k) = \mathrm{softplus}(-w_{\mathrm{la}} + \mathrm{net}(k))$。动机是论文 §5 (line 585) 的 "deeper neural architectures for enhanced temporal modelling"，但未作为正式开放问题。结果 MAPE 1.10%，劣于 baseline——6 个训练 k 值不足以训练有意义的时变函数，确认了该方向在数据量更大时才有实际价值。
